...
fn default_wallpaper_path() -> String {
    "~/.config/sone-papdi/wallpaper.jpg".into()
}
...
/// Returns the default config file path: `~/.config/sone-papdi/config.toml`.
...
"##;
    ...
    async fn test_config_manager_creates_default() {
        ...
        let config_path = dir.path().join(".config/sone-papdi/config.toml");
    ...
    async fn test_config_manager_loads_existing() {
        let dir = tempfile::tempdir().unwrap();
        let config_dir = dir.path().join(".config/sone-papdi");
