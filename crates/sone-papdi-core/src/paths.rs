...
/// User data directory for sone-papdi.
pub fn data_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("XDG_DATA_HOME") {
        return PathBuf::from(dir).join("sone-papdi");
    }
    dirs_next::data_dir()
        .map(|p| p.join("sone-papdi"))
        .unwrap_or_else(|| PathBuf::from("/tmp/sone-papdi-data"))
}

/// User config directory for sone-papdi.
pub fn config_dir() -> PathBuf {
    ...
    dirs_next::config_dir()
        .map(|p| p.join("sone-papdi"))
        .unwrap_or_else(|| PathBuf::from("/tmp/sone-papdi-config"))
}
...
mod tests {
    ...
    fn sockets_are_in_runtime_dir() {
        with_runtime_dir(None, || {
            let rt = runtime_dir();
            assert_eq!(daemon_socket(), rt.join("sone-papdi.sock"));
            assert_eq!(clipboard_socket(), rt.join("sone-papdi-clipboard.sock"));
            assert_eq!(bar_socket(), rt.join("sone-papdi-bar.sock"));
        });
    }
