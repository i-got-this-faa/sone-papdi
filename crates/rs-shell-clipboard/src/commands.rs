...
use rs_shell_core::events::{ClipboardEvent, ShellEvent};
use std::sync::Arc;
...
impl ClipboardCommander {
    ...
    pub fn new(db: Arc<Mutex<Database>>, bus: EventBus, config: ClipboardConfig) -> Self {
        Self { db, bus, config }
    }
    ...
    pub async fn copy(&self, id: i64) -> Result<()> {
        ...
        self.set_clipboard(&entry.mime, &content).await?;
    ...
    /// List entries and emit a `SearchResult` event.
    pub async fn list(&self, limit: Option<usize>) -> Result<Vec<rs_shell_core::events::ClipEntry>> {
        let limit = limit.unwrap_or(self.config.max_entries);
        let db = self.db.lock().await;
    ...
    pub async fn search(&self, query: String, limit: Option<usize>) -> Result<Vec<rs_shell_core::events::ClipEntry>> {
        let limit = limit.unwrap_or(self.config.max_entries);
        let db = self.db.lock().await;
        let results = db.list_entries(&query, limit, 0)?;

        self.bus.publish(ShellEvent::Clipboard(ClipboardEvent::SearchResult {
            query: query.clone(),
    ...
    pub async fn set_pinned(&self, id: i64, pinned: bool) -> Result<()> {
        ...
        self.bus.publish(ShellEvent::Clipboard(ClipboardEvent::PinnedToggled { id, pinned }));
        Ok(())
    }

    async fn set_clipboard(&self, mime: &str, content: &str) -> Result<()> {
