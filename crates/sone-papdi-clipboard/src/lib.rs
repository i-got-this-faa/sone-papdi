...
//!                  │  └─────────────────────────┘   │
//!                  └──────────────┬────────────────┘
//!                                 │ /run/user/UID/sone-papdi-clipboard.sock
//!                  ┌──────────────▼────────────────┐
...
use sone_papdi_core::ipc::{send_message, IpcMessage, IpcResponse, ShellCommand};
use sone_papdi_core::paths;
use std::sync::Arc;
...
///
/// Socket: `{runtime_dir}/sone-papdi-clipboard.sock`
...
async fn handle_client(
    ...
    loop {
        let msg = match sone_papdi_core::ipc::recv_message(&mut stream).await {
            Ok(msg) => msg,
